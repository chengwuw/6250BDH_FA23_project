...
if __name__ == '__main__':
    ...
    # unzip file
    print('Unzip file...')
    with gzip.open(hp.data_dir + 'D_ITEMS.csv.gz') as f:
        defs = pd.read_csv(f, usecols=dtype.keys(), dtype=dtype)
    ...
    with gzip.open(hp.data_dir + 'CHARTEVENTS.csv.gz', 'rt') as f:
      # Read the decompressed CSV file using pd.read_csv
      chart_event = pd.read_csv(f, usecols=dtype.keys(), dtype=dtype, chunksize=chunksize)
      for df in tqdm(chart_event):
          # pass
      # for df in tqdm(chart_event):
